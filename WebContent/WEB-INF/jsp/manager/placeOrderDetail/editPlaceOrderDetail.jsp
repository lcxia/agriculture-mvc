                    ...
                    <div class="box col-md-12">
                        <div class="box-inner">
                            <div class="box-header well" data-original-title="">
                                <h2>
                                    <i class="glyphicon glyphicon-edit"></i> 销售订单明细信息
                                </h2>
                            ...
                            <div class="box-content">
                                <form:form modelAttribute="placeOrderDetailForm" action="editPlaceOrderDetail" method="post">
                                	<form:hidden path="placeOrderId" value="${placeOrderDetailForm.placeOrderId}" />
                                	<form:hidden path="placeOrderDetailId" value="${placeOrderDetailForm.placeOrderDetailId}" />
                                	<div class="alert alert-info">${message}<form:errors path="*"></form:errors></div>
                                    <table class="table table-bordered responsive">
                                        <tr>
											<td style="background-color: #f9f9f9;">供应商</td>
											<td>
												<form:select path="supplierId" itemValue="${placeOrderDetailForm.supplierId}">
													<form:options id="supplierId" items="${supplierList}" itemLabel="label" itemValue="value"/>
												</form:select>
											</td>
											<td style="background-color: #f9f9f9;">品牌</td>
											<td>
												<form:select path="brandId" itemValue="${placeOrderDetailForm.brandId}">
													<form:options id="brandId" items="${brandList}" itemLabel="label" itemValue="value"/>
												</form:select>
											</td>
											<td style="background-color: #f9f9f9;">商品名称</td>
											<td>
												<form:select path="commodityId" itemValue="${placeOrderDetailForm.commodityId}">
													<form:options id="commodityId" items="${commodityList}" itemLabel="label" itemValue="value"/>
												</form:select>
											</td>
										</tr>
										<tr>
											<td style="background-color: #f9f9f9;">规格</td>
											<td><span id="specifications"></span></td>
											<td style="background-color: #f9f9f9;">单位</td>
											<td><span id="unit"></span></td>
											<td style="background-color: #f9f9f9;">重量</td>
											<td><span id="weight"></span></td>
										</tr>
										<tr>
											<td style="background-color: #f9f9f9;">基准价格</td>
											<td><span id="benchmarkPrice"></span></td>
											<td style="background-color: #f9f9f9;">销售指导价格</td>
											<td><span id="guidePrice"></span></td>
											<td style="background-color: #f9f9f9;">终端零售价格</td>
											<td><span id="retailPrice"></span></td>
										</tr>
										<tr>
											<td style="background-color: #f9f9f9;">库存</td>
											<td><span id="stock"></span></td>
											<td style="background-color: #f9f9f9;">购买数量</td>
											<td><form:input path="purchaseCount" cssClass="form-control" cssErrorClass="form-control error" value="${placeOrderForm.purchaseCount}"/></td>
											<td style="background-color: #f9f9f9;">备注</td>
											<td><input name="note" class="form-control" value="${placeOrderDetailForm.note}"/></td>
										</tr>
                                    </table>
                                    <div class="input-group has-success col-md-12" align="right">
                                        <button type="submit" class="btn btn-success">提 交</button>
                                        &nbsp;&nbsp;&nbsp;&nbsp;
                                        <a type="button" class="btn btn-danger" href="initPlaceOrderDetail?placeOrderId=${placeOrderDetailForm.placeOrderId}">
	...
	$(function(){
		$("#brandId").change(function(){
			...
			$.ajax({
				...
				success: function(data){
					$("#commodityId").empty();
					$.each(data.commoditys, function(index, item){
						var myOption = document.createElement("option"); 
						myOption.value = item.value; 
						myOption.text = item.label; 
						$("#commodityId").append(myOption);
					});
					$("#commodityId").change();
				}
			});
		...
		$("#commodityId").change(function(){
			...
			$.ajax({
				...
				success: function(data){
					...
					$("#specifications").append(data.specifications);
					$("#unit").empty();
					$("#unit").append(data.unit);
					$("#weight").empty();
					$("#weight").append(data.weight);
					$("#benchmarkPrice").empty();
					$("#benchmarkPrice").append(data.benchmarkPrice);
					$("#guidePrice").empty();
					$("#guidePrice").append(data.guidePrice);
					$("#retailPrice").empty();
					$("#retailPrice").append(data.retailPrice);
					$("#stock").empty();
